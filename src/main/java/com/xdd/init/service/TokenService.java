    ...
    public SysUser getLoginUser(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            try {
                ...
                // 解析对应的权限以及用户信息
                Long uuid = (Long) claims.get(Constants.LOGIN_USER_KEY);
...
//    {
//        if (user !=null && StringUtils.isNotEmpty(loginUser.getToken()))
//        {
...
//    }
     ...
     * 删除用户身份信息
     */
    public void delLoginUser(Long uid) {
        if (uid != null) {
            String userKey = getTokenKey(uid);
    ...
    public String createToken(SysUser sysUser) {
        ...
        claims.put(Constants.LOGIN_USER_KEY, sysUser.getUserId());
    ...
    public void refreshToken(SysUser user) {
        // 根据uuid将loginUser缓存
        String userKey = getTokenKey(user.getUserId());
